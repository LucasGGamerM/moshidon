	...
	public static ArrayList<StatusDisplayItem> buildItems(BaseStatusListFragment<?> fragment, Status status, String accountID, DisplayItemsParent parentObject, Map<String, Account> knownAccounts, FilterContext filterContext, int flags){
		...
		if((flags & FLAG_NO_HEADER)==0){
			if(status.reblog!=null){
				...
				items.add(new ReblogOrReplyLineCustomStatusDisplayItem(parentID, fragment, fragment.getString(R.string.user_boosted, status.account.getDisplayName()), status.account.emojis, R.drawable.ic_repeat_wght700_20px, null, null, fragment.getString(R.string.user_boosted, status.account.displayName), status.account));
				...
				items.add(new ReblogOrReplyLineCustomStatusDisplayItem(parentID, fragment, fragment.getString(R.string.in_reply_to, account.getDisplayName()), account.emojis, R.drawable.ic_reply_wght700_20px, null, null, fragment.getString(R.string.in_reply_to, account.displayName), account));
			}
